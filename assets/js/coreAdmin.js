                ...
                .done(function(data) {
                    ...
                    if (dato['valid']) {
                        ...
                        acciones = '<a href="#modal-editar-usuario" class="tip modificarUsuario" data-id="' + dato['usuario'][0]['idUsuario'] + '" data-toggle="modal" style="color: #4e73df;" data-original-title="Editar Usuario"><i class="fa fa-edit"></i></a> &nbsp;' +
                            '<a class="tip" role="button" onclick="eliminar_usuario(' + dato['usuario'][0]['idUsuario'] + ')" data-toggle="modal" style="color: #4e73df;" data-original-title="Eliminar"><i class="fas fa-trash"></i></a>';
            ...
            $.ajax({
                ...
                .done(function(data) {
                    ...
                    if (dato['valid']) {
                        ...
                        $('#horaHasta_formSolicitudEnvio').attr('disabled', 'disabled');

                        //--- Asignaciones ---//
                        document.getElementById("idPedido_formSolicitudEnvio").value = dato['idPedido'];
                        ...
                        var row = table_add_pedido.row.add([
                            dato['idDetallePedido'],
                            dato['nombreReceptor'],
                            dato['comuna'],
                            dato['cantidad'],
                        ...
                        table_add_pedido.row(row).column(6).nodes().to$().addClass('text-center');

                        //--- Vaciado de campos para continuar agregando otros pedidos ---//
                        ...
                        document.getElementById("cantidad_pedidos_formSolicitudEnvio").value = parseInt(cantidadPedidos) + 1;
                ...
                //activar tabla cuando haya al menos un pedido
                listado_pedidos.setAtribute("style", "display=block;");
                listado_add_pedidos.setAtribute("style", "display=block;");
    ...
    $('#add_pedido_js').click(function(e) {
        ...
        if (cantidadPedidos > 0) {
            $.ajax({
                    ...
                    type: 'POST',
                    data: { idPedido: idPedido, montoTotal: montoTotal, cantidadPedidos: cantidadPedidos },
                })
                .done(function(data) {
                    var dato = JSON.parse(data);
    ...
    $.ajax({
        ...
        .done(function(data) {
            ...
            if (dato['valid']) {
                ...
                tableListadoAddPedidos.row('#' + idDetallePedido).remove().draw();

                //--- Restamos uno en el pedido y el precio ---//
                var montoTotal = $('#montoTotal_formSolicitudEnvio').val();
                var cantidadPedidos = $('#cantidad_pedidos_formSolicitudEnvio').val();
